...
4. Move a valid `credentials.json`, `riwhale.config`, and `.mapbox_token` to the newly-created `conf` directory<br/>(if you have a valid `.token.pickle` move that to `conf` and _skip_ steps 5-7).<br/>
